...
pub async fn revoke_manga_chapter(
    ...
    let manga_info = database.find_cached_manga_information(chapter.manga_id()).await?;
    let manga_title = manga_info.as_ref().and_then(|m| m.title.as_deref()).unwrap_or("Unknown");
